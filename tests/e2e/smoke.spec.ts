...
test(
  'admin publishes content and learner completes the core journey',
  async ({ page }) => {
    ...
    await page
      .getByLabel('Câu hỏi')
      .fill('Đâu là đáp án đúng của câu kiểm thử?');
    await page
    ...
    await expect(
      page.getByText('Đâu là đáp án đúng của câu kiểm thử?'),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('heading', { name: 'Bài học 1' }),
    ).toBeVisible();
