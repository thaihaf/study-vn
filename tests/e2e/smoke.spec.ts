...
const learnerEmail = 'e2e-learner@ci.example.test';
const learnerPassword = 'LearnerPassword123!';
const courseTitle = 'Khóa học kiểm thử E2E';
const assessmentTitle = 'Bài luyện kiểm thử E2E';
...
test('admin publishes content and learner completes the core journey', async ({ page }) => {
  ...
  await page.getByLabel('Câu hỏi').fill('Đâu là đáp án đúng của câu kiểm thử?');
  await page
    .getByLabel('Lựa chọn / đáp án Đúng-Sai')
    .fill('* Đáp án đúng\nĐáp án sai');
  await page.getByLabel('Giải thích').fill('Đáp án đầu tiên được đánh dấu đúng.');
  await page.getByLabel('Trạng thái').selectOption('PUBLISHED');
  await page.getByRole('button', { name: 'Tạo câu hỏi' }).click();
  await expect(page.getByText('Đâu là đáp án đúng của câu kiểm thử?')).toBeVisible();

  await page.goto('/admin/assessments');
  await page.getByLabel('Tên').fill(assessmentTitle);
  await page.getByLabel('Mô tả').fill('Bài luyện dùng trong kiểm thử đầu cuối.');
  await page.getByRole('button', { name: 'Tạo bài đánh giá' }).click();
  const assessmentCard = page.locator('article.card').filter({ hasText: assessmentTitle });
  await assessmentCard.locator('select[name="questionId"]').selectOption({ index: 1 });
  ...
  await expect(page.getByRole('heading', { name: /Kết quả: 100%/ })).toBeVisible();
