...
import { applyGenerationJob } from '@/modules/ai/apply-actions';
            ...
            >
              ...
              {job.outputSnapshot && (
                <details open={job.id === selectedJobId}>
                  <summary>
                    <b>Xem output có cấu trúc</b>
                  </summary>
                  <pre className="code-block">
                    <code>{JSON.stringify(job.outputSnapshot, null, 2)}</code>
                  </pre>
                </details>
              )}
              {job.kind === 'LESSON' && job.artifacts[0] && (
                <details>
              ...
              <div className="builder-row">
                ...
                {job.status === 'SUCCEEDED' &&
                  ...
                  job.kind === 'LESSON' &&
                  !lessonApplied && (
                    <form action={applyLessonGeneration}>
                      <input type="hidden" name="jobId" value={job.id} />
                      <button className="btn">Áp dụng vào bài học</button>
                    </form>
                  )}
                {job.status === 'SUCCEEDED' &&
                  job.outputSnapshot &&
                ...
                {(lessonApplied ||
                  (job.kind !== 'LESSON' && job.targetEntityId)) && (
                  <span className="status">Đã áp dụng</span>
                )}
              </div>
            </article>
          );
