...
export default async function Preview({ params }: { params: Promise<{ id: string }> }) {
              ...
              lessons: {
                ...
                include: {
                  blocks: {
                    orderBy: { position: 'asc' },
                    include: {
                      citations: { include: { chunk: { include: { source: true } } } },
                    },
                  },
                },
              },
  ...
  return (
    <div className="preview-shell">
      <span className="status">Bản xem trước quản trị · {version.status} · v{version.versionNumber}</span>
      <h1>{course.title}</h1>
      ...
      {version.modules.map((courseModule) => (
        <section className="preview-module" key={courseModule.id}>
          ...
          {courseModule.description && <p className="muted">{courseModule.description}</p>}
