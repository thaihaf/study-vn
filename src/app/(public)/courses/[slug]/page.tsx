...
export default async function CoursePage({
  ...
  const next =
    lessons.find(
      (lesson) => !progress.some((item) => item.lessonId === lesson.id),
    ) ?? lessons[0];
  const percent = enrollment
    ? calculateProgress(progress.length, lessons.length)
    : 0;
  ...
  return (
    <div className="page container">
      ...
      <p>
        ...
        · {lessons.length} bài · {course.assessments.length} bài luyện
      </p>
      {enrollment && next ? (
        <div className="card">
          <b>Tiến độ {percent}%</b>
          ...
          />
          <p>
            <Link className="btn" href={`/learn/${course.slug}/${next.slug}`}>
              {percent === 100 ? 'Học lại từ đầu' : `Tiếp tục: ${next.title}`}
            </Link>
          </p>
        </div>
      ) : session ? (
      ...
      <div className="grid">
        {course.currentPublishedVersion.modules.map((courseModule) => (
          <section className="card" key={courseModule.id}>
            <h3>
              {courseModule.position + 1}. {courseModule.title}
            </h3>
          ...
          <div className="grid">
            {course.assessments.map((assessment) => (
              ...
              >
                ...
                <p className="muted">{assessment.type}</p>
              </Link>
            ))}
          </div>
        </section>
