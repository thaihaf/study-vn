...
async function resolveBankAndTopic(bankTitle: string, topicName: string) {
  const bank = await db.questionBank
    .upsert({
      where: {
        id: bankTitle.startsWith('id:') ? bankTitle.slice(3) : '__never__',
      },
      update: {},
    ...
    .catch(async () => {
      const existing = await db.questionBank.findFirst({
        where: { title: bankTitle || 'Ngân hàng chung' },
      });
      return (
        existing ??
        db.questionBank.create({
          data: { title: bankTitle || 'Ngân hàng chung' },
        })
      );
    });
  ...
  if (!topic) {
    ...
    topic =
      existsBySlug ??
      (await db.topic.create({ data: { name: trimmedTopic, slug: base } }));
  }
  return { bank, topic };
}
...
export async function createQuestion(form: FormData) {
  ...
  if (raw.rubric.trim()) {
    try {
      rubricJson = JSON.parse(raw.rubric);
    } catch {
      rubricJson = { criteria: raw.rubric };
    }
  }
